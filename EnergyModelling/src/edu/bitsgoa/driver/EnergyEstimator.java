...
public class EnergyEstimator extends AbstractHandler implements IHandler{
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if(!pathToCurrentFile.endsWith(".cu")){
			DisplayCustomConsole.display("Cannot Perform Energy Estimation on a non .cu file",true);
		}
		else{
		...
		cmd[1]="-ptx";
		cmd[2]=fileName;
		BetterRunProcess process=new BetterRunProcess(cmd,builderDirectory,1,0,0,"");
		process.run();
	...
	public boolean isEnabled(){
		if(Preparation.used==true && Parameters.timesused>0) return true;
		else return false;
	}
