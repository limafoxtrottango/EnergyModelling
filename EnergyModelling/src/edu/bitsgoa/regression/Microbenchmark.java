...
public class Microbenchmark implements Runnable {
	private boolean[] existenceArray;
	...
	public static void runInstructions(String
			cmd,String path_executables,String fileName){
		...
		for(int i=0;i<50;i++){
			if(i==0){
				process.setAppend(0);
				process.setCmd(command);
				process.setPath(path_executables);
				process.setPrintToConsole(0);
				process.setPrintToExternalFile(1);
	...
	public static void maxFreqCommandRun(String fileName){
		...
		BetterRunProcess process=new BetterRunProcess(cmd,Preparation.path_home,0,1,0,"av_"+fileName);
		process.run();
	}
